...
class CTxtWinHost : public ITextHost /// ITextHost是微软提供的接口
{
public:
    CTxtWinHost();
	...
	BOOL CanPaste(UINT nFormat = 0)
	{
		if (NULL == pserv)
			return FALSE;
		LRESULT lRes = 0;
		pserv->TxSendMessage(EM_CANPASTE, nFormat, 0L, &lRes); /// 发送消息给COM对象
		return (BOOL)lRes;
	}
...
private:
    ...
    ITextServices	*pserv;		    // pointer to Text Services object /// 这个也是微软提供的接口
    // Properties
...
HRESULT CreateHost(RichEdit *re, const CREATESTRUCT *pcs, CTxtWinHost **pptec) /// 这是一个单独的函数，不属于任何类
{
    HRESULT hr = E_FAIL;
    //GdiSetBatchLimit(1);
...
BOOL CTxtWinHost::Init(RichEdit *re, const CREATESTRUCT *pcs)
{
	...
	//½â¾ö32Î»ÏµÍ³ÏÂ¿ç´°¿Ú¼äÍÏ×§ÎÄ×ÖÔÚwin7¼°win7ÒÔÏÂÏµÍ³ÉÏ»á³öÏÖÖØ¸´µÄÎÊÌâ£¨64Î»ÔÝ²»ÐÞ¸´£© lty 20170714
	...
	if (!(::GetFileAttributesW(edit_dll.c_str()) != INVALID_FILE_ATTRIBUTES))
		edit_dll = L"msftedit.dll";
#endif
	hmod = LoadLibraryW(edit_dll.c_str()); //msftedit.dll /// 加载动态库
	if (hmod)
	{
		TextServicesProc = (PCreateTextServices)GetProcAddress(hmod,"CreateTextServices"); /// 获得函数指针
	}
...
{
	ASSERT(FALSE); //ÔÝÊ±×¢ÊÍµô£¬²»ÖªµÀÕâ´úÂëÓÐÉ¶ÓÃ   by panqinke 2014.5.6
...
void RichEdit::OnKeyDown(EventArgs& event)
{
	if (event.wParam == VK_RETURN && ::GetAsyncKeyState(VK_SHIFT) >= 0)	{
		...
		if (!strClipText.empty()) {
			...
			SetClipBoardText(strNum); //ÐÞ¸Ä¼ôÇÐ°åÄÚÈÝÎª´¿Êý×Ö
			nbase::ThreadManager::PostTask([strClipText]() { SetClipBoardText(strClipText); }); //Õ³ÌùÍêºóÓÖ°Ñ¼ôÇÐ°åÄÚÈÝ¸Ä»ØÀ´
		}
	}
...
//¸ù¾ÝpointÀ´hittest×Ô¶¨ÒålinkµÄÊý¾Ý£¬·µ»Øtrue±íÊ¾ÔÚlinkÉÏ£¬infoÊÇlinkµÄ×Ô¶¨ÒåÊôÐÔ
...
void SetClipBoardText(const std::wstring &str)
{
	...
	::memcpy(lpStr, str.c_str(), len * sizeof(wchar_t)); //°ÑÊý¾Ý¿½±´¿¼È«¾ÖÄÚ´æÖÐ
	lpStr[len] = wchar_t(0); //×Ö·û´®Ä©Î²ÉèÎª'\0'
	::GlobalUnlock(hMem); //ÊÍ·ÅËø 

	::SetClipboardData(CF_UNICODETEXT, hMem); //°ÑÄÚ´æÖÐµÄÊý¾Ý·Åµ½¼ôÇÐ°åÉÏ
	::CloseClipboard(); //¹Ø±Õ¼ôÇÐ°å	
}
